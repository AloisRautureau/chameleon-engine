...
impl CastlingRights {
    pub fn new() -> CastlingRights { CastlingRights(0b1111) }
    pub fn from_str(s: &str) -> CastlingRights {
        let mut res = CastlingRights(0);
        if s.contains('K') { res.0 |= 0b1000 }
        if s.contains('Q') { res.0 |= 0b0100 }
        if s.contains('k') { res.0 |= 0b0010 }
        if s.contains('q') { res.0 |= 0b0001 }
        res
    }
