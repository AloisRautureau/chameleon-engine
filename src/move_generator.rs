...
pub fn generate(board: &Board, gen_type: GenType) -> MoveList {
    ...
    let us = board.side_to_move(); let them = us.opposite();
    let us_index = us as usize;

    let king_sq = if let Some(sq) = board.king_square(us) { sq } else { return move_list };
    let occupancy = board.get_occupancy_bitboard();
    let opponents = board.get_color_bitboard(them);

    let mut push_mask = if gen_type == GenType::Captures { Bitboard::EMPTY } else { !occupancy };
            ...
            let attacks = match board.piece_type_on(pinned_sq).unwrap() {
                PieceType::Pawn => {
                    ...
                    if (cap & pin_ray).is_set(board.en_passant_target().unwrap_or(64)) {
                        move_list.push(Move::new_en_passant(pinned_sq, board.en_passant_target().unwrap()))
                    }
                    Bitboard::EMPTY
                },
                PieceType::Bishop => Bitboard::bishop_attacks(pinned_sq, occupancy),
                PieceType::Rook => Bitboard::rook_attacks(pinned_sq, occupancy),
                PieceType::Queen => Bitboard::bishop_attacks(pinned_sq, occupancy) | Bitboard::rook_attacks(pinned_sq, occupancy),
                _ => Bitboard::EMPTY
            };
