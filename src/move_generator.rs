...
pub fn generate(board: &Board, gen_type: GenType) -> MoveList {
    let mut info = MoveGenInfo {
        ...
        king_square: if let Some(sq) = board.king_square(board.side_to_move()) {
            sq
        } else {
            return MoveList::default();
        },
        side: board.side_to_move(),
        occupancy: board.get_occupancy_bitboard(),
        opponents: board.get_color_bitboard(board.side_to_move().opposite()),
        push_targets: if gen_type == GenType::Captures {
            Bitboard::EMPTY
        } else {
    ...
    let mut king_attackers =
        board.attackers_of_square(info.king_square, info.side.opposite(), Bitboard::EMPTY);
    ...
    if let Some(checker_square) = king_attackers.next() {
        ...
        if king_attackers.next().is_some() {
            return info.move_list;
        }
        ...
        {
            info.push_targets =
                Bitboard::get_ray(info.king_square, checker_square) & !info.occupancy;
        } else {
...
fn pawn_moves(board: &Board, info: &mut MoveGenInfo) {
    let (m, pre_promo_rank) = if info.side == Color::Black {
        (-1, Bitboard::RANKS[1])
    } else {
        (1, Bitboard::RANKS[6])
    };
    ...
    for target in Bitboard::pawn_pushes(pawns_bb, !info.occupancy, info.side) & info.push_targets {
        info.move_list
            .push(Move::new_quiet(target - (8 * m as usize), target))
    }
    for target in
        Bitboard::pawn_double_pushes(pawns_bb, !info.occupancy, info.side) & info.push_targets
    {
        info.move_list
            .push(Move::new_double_push(target - (16 * m as usize), target))
    }
    for target in Bitboard::pawn_west_attacks(pawns_bb, info.side) & info.capture_targets {
        info.move_list
            .push(Move::new_capture(target - (7 * m as usize), target))
    }
    for target in Bitboard::pawn_east_attacks(pawns_bb, info.side) & info.capture_targets {
        info.move_list
            .push(Move::new_capture(target - (9 * m as usize), target))
    }
}
...
fn pawn_promotions(board: &Board, info: &mut MoveGenInfo) {
    ...
    let pawns_bb =
        board.get_piece_bitboard(PieceType::Pawn, info.side) & !info.pinned_pieces & pre_promo_rank;
...
fn king_moves(attacks: Bitboard, info: &mut MoveGenInfo) {
    ...
    for target in moves & info.push_targets {
        info.move_list
            .push(Move::new_quiet(info.king_square, target))
    }
    for target in moves & info.capture_targets {
        info.move_list
            .push(Move::new_capture(info.king_square, target))
    }
}
...
fn piece_move(board: &Board, piece_type: PieceType, info: &mut MoveGenInfo) {
    match piece_type {
        ...
        PieceType::Bishop | PieceType::Rook | PieceType::Queen => {
            specific_slider_moves(board, piece_type, info)
        }
        _ => (),
    }
}
...
fn en_passant(board: &Board, info: &mut MoveGenInfo) {
    if let Some(target) = board.en_passant_target() {
        let removed_piece_square = if board.side_to_move() == Color::White {
            target - 8
        } else {
            target + 8
        };
        if !info.push_targets.is_set(target) && !info.capture_targets.is_set(removed_piece_square) {
            return;
        }

        // Checks for extermely rare cases where the en passant capture might leave the king in check
        let rook_sliders = board
            .get_piece_bitboard(PieceType::Rook, board.side_to_move().opposite())
            | board.get_piece_bitboard(PieceType::Queen, board.side_to_move().opposite());
        ...
        let origins = Bitboard::pawn_attacks(
...
fn castling(board: &Board, attacks: Bitboard, info: &mut MoveGenInfo) {
    ...
    let kingside_occupancy =
        info.occupancy & Bitboard::CASTLING_OCCUPANCY_MASKS[info.side as usize][0];
    let kingside_attacked = attacks & Bitboard::CASTLING_ATTACKED_MASKS[info.side as usize][0];
    let queenside_occupancy =
        info.occupancy & Bitboard::CASTLING_OCCUPANCY_MASKS[info.side as usize][1];
...
fn pinned_pieces_moves(board: &Board, info: &mut MoveGenInfo) {
    let (push_targets, capture_targets, pinned_pieces) =
        (info.push_targets, info.capture_targets, info.pinned_pieces);
