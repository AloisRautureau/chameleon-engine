...
use crate::search::{ SearchFramework, Search, SearchOptions };
...
impl UCI {
    pub fn run() {
        ...
        while let Ok(line) = rl.readline("uci> ") {
            ...
            match instance.handle_command(&line) {
                ...
                Err(UCIErrCode::BadCommand(cmd)) => eprintln!("Unknown or badly formed UCI command: {}", cmd),
                Err(UCIErrCode::BadMove(mv)) => eprintln!("Badly formatted or illegal move: {}", mv),
                Err(UCIErrCode::MissingArg(arg)) => eprintln!("Missing an argument: {} {} <- here", line.trim(), arg),
                _ => ()
            }
        }
    }
    ...
    fn handle_command(&mut self, line: &str) -> Result<UCIOkCode, UCIErrCode> {
        ...
        let cmd = if let Some(c) = args.next() { c } else { return Err(UCIErrCode::NoCommand) };
        match cmd {
            "uci" => {
            ...
            "setoption" => (),
            "ucinewgame" => self.board = Board::new("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"),
            "position" => {
                match args.next() {
                    Some("startpos") => self.board = Board::new("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"),
    ...
    fn parse_go_args<I: Iterator<Item = String>>(&self, args: I) -> SearchOptions {
        let valid_args = ["searchmoves", "ponder", "wtime", "btime", "winc", "binc", "movestogo", "depth", "nodes", "mate", "movetime", "infinite"];
        ...
        let (clock, increment) = if self.board.side_to_move() == Color::White {
            (
                arg_value_map.get("wtime").map(|d| Duration::from_millis(d.parse::<u64>().unwrap())),
                arg_value_map.get("winc").map(|d| Duration::from_millis(d.parse::<u64>().unwrap()))
            )
        } else {
            (
                arg_value_map.get("btime").map(|d| Duration::from_millis(d.parse::<u64>().unwrap())),
                arg_value_map.get("binc").map(|d| Duration::from_millis(d.parse::<u64>().unwrap()))
            )
        };
    ...
    fn args_regex() -> Regex { Regex::new(r#"(".*?"|[^"\s]+)"#).unwrap() }
}
